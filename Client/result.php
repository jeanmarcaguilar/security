    ...
    <div class="content">
      ...
      <div class="card tbl-card">
        <div class="tbl-bar">
          ...
          <div class="frow">
            <div style="display:flex;align-items:center;gap:.5rem;font-size:.75rem;color:var(--muted2)">
              <span>Total: <?php echo count($history); ?> assessments</span>
              <?php if (count($history) > 1): ?>
                <span>•</span>
                <span>Avg: <?php echo number_format(array_sum(array_column($history, 'score')) / count($history), 1); ?>%</span>
                <span>•</span>
                <span>Best: <?php echo max(array_column($history, 'score')); ?>%</span>
              <?php endif; ?>
            </div>
            <button class="btn btn-p btn-sm" onclick="exportResults()">⬇ Export</button>
          </div>
        </div>
        <div class="tw"><table><thead><tr><th>#</th><th>Date</th><th>Score</th><th>Rank</th><th>Trend</th><th>Password</th><th>Phishing</th><th>Device</th><th>Network</th></tr></thead><tbody id="history-tbl-body"></tbody></table></div>
      </div>
      ...
      <div class="card" style="text-align: center; padding: 3rem;">
        ...
        <button class="btn btn-p" onclick="window.location.href='assessment.php'" style="margin-top: 1rem;">Start Assessment</button>
      </div>
      <?php endif; ?>
...
<script>
// PHP session data exposed to JS
const latestAssessment = <?php echo json_encode($latest_assessment); ?>;
const history = <?php echo json_encode($history); ?>;

async function doLogout() {
    const res = await fetch('index.php?action=logout', { method: 'POST', credentials: 'same-origin' });
...
function populateHistoryTable() {
    ...
    tbody.innerHTML = history.map((a, index) => {
        const assessmentNumber = history.length - index;
        const trend = index < history.length - 1 ? 
            (a.score > history[index + 1].score ? '📈' : a.score < history[index + 1].score ? '📉' : '➡️') : 
            '🆕';
        return `
        <tr>
            <td><span style="font-family:var(--mono);color:var(--muted2);font-weight:600">#${assessmentNumber}</span></td>
            <td>${new Date(a.created_at).toLocaleDateString()}</td>
            <td>${a.score}%</td>
            <td><span class="rank r${a.rank}">${a.rank}</span></td>
            <td><span style="font-size:1rem">${trend}</span></td>
            <td><div class="sbw"><div class="sbb"><div class="sbf" style="width:${a.password_score}%;background:var(--blue)"></div></div><span class="sbn">${a.password_score}%</span></div></td>
            <td><div class="sbw"><div class="sbb"><div class="sbf" style="width:${a.phishing_score}%;background:var(--green)"></div></div><span class="sbn">${a.phishing_score}%</span></div></td>
            <td><div class="sbw"><div class="sbb"><div class="sbf" style="width:${a.device_score}%;background:var(--teal)"></div></div><span class="sbn">${a.device_score}%</span></div></td>
            <td><div class="sbw"><div class="sbb"><div class="sbf" style="width:${a.network_score}%;background:var(--purple)"></div></div><span class="sbn">${a.network_score}%</span></div></td>
        </tr>`;
    }).join('');
}
...
function drawScoreCircle() {
    ...
    if (circle) circle.style.background = `conic-gradient(var(--blue) ${deg}deg, var(--bg2) ${deg}deg)`;
}
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['rgba(59,139,255,.7)', 'rgba(16,217,130,.7)', 'rgba(0,212,170,.7)', 'rgba(123,114,240,.7)'],
                borderRadius: 6
            }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { max: 100, grid: { color: 'rgba(255,255,255,.05)' }, ticks: { color: '#8898b4' } }, x: { grid: { display: false }, ticks: { color: '#8898b4' } } } }
    });
}
...
function initRadarChart() {
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: [latestAssessment.password_score, latestAssessment.phishing_score, latestAssessment.device_score, latestAssessment.network_score],
                backgroundColor: ['rgba(59,139,255,.8)', 'rgba(16,217,130,.8)', 'rgba(0,212,170,.8)', 'rgba(123,114,240,.8)'],
                borderWidth: 0
            }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: '#8898b4', font: { size: 10 } } } } }
    });
}
...
function initProgressChart() {
    ...
    new Chart(ctx, {
        type: 'line',
        data: {
            labels: reversed.map(a => new Date(a.created_at).toLocaleDateString()),
            datasets: [{
                label: 'Score',
                data: reversed.map(a => a.score),
                borderColor: 'var(--blue)',
                backgroundColor: 'rgba(59,139,255,.1)',
                fill: true,
                tension: 0.4,
        ...
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { max: 100, min: 0, grid: { color: 'rgba(255,255,255,.05)' }, ticks: { color: '#8898b4' } }, x: { grid: { display: false }, ticks: { color: '#8898b4' } } } }
    });
}

function exportResults() {
    if (!latestAssessment) return;
    const text = `CyberShield Assessment Report
