...
<title>Compliance — CyberShield</title>
...
<div class="sec-hdr"><h2>Compliance Checklist</h2><p>Per-vendor action items based on their identified weak areas.</p></div>
<div class="card" style="padding:1.25rem 1.5rem;margin-bottom:1.25rem">
  <div class="tbl-bar" style="margin-bottom:1rem">
    <h3 style="font-family:var(--mono);font-size:.68rem;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted2)">Select Vendor</h3>
    <select class="fsel" id="comp-v" onchange="renderComp()" style="min-width:220px;padding:.45rem .85rem;border-radius:8px"></select>
  </div>
...
const CHECKS={
  'Access Control':['Review user permissions quarterly','Enforce MFA for all accounts','Audit privileged access logs'],
  'Network Security':['Patch all firewall rules','Enable IDS/IPS monitoring','Review VPN access policies'],
  'Data Encryption':['Encrypt all data at rest','Use TLS 1.3 for transit','Rotate encryption keys annually'],
  'Compliance':['Complete SOC 2 audit','Update privacy policy','Train staff on GDPR'],
  'Incident Response':['Test incident response plan','Document breach procedures','Run tabletop exercise'],
  'Physical Security':['Review access card logs','Inspect server room locks','Update visitor procedures']
};
function pageInit(){
  ...
  all.forEach(cat=>{
    ...
    html+=`<div style="margin-bottom:1rem">
      <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.5rem">
        <span style="font-family:var(--mono);font-size:.65rem;letter-spacing:1px;text-transform:uppercase;color:${isWeak?'var(--red)':'var(--green)'};font-weight:600">${isWeak?'⚠':'✓'} ${cat}</span>
        ${isWeak?'<span style="font-family:var(--mono);font-size:.58rem;padding:.15rem .4rem;background:rgba(255,59,92,.1);color:var(--red);border-radius:4px">Action Required</span>':''}
      </div>
      ${CHECKS[cat].map((item,i)=>`<div style="display:flex;align-items:center;gap:.65rem;padding:.42rem .65rem;border-radius:6px;${isWeak?'background:rgba(255,59,92,.04);border:1px solid rgba(255,59,92,.1)':'background:rgba(16,217,130,.04);border:1px solid rgba(16,217,130,.08)'}">
        <input type="checkbox" ${isWeak?'':'checked'} style="flex-shrink:0">
        <span style="font-size:.8rem;${isWeak?'color:var(--text)':'color:var(--muted2)'}">${item}</span>
      </div>`).join('')}
    </div>`;
  });
