...
const RANK_CFG = {
  ...
  B:{color:'#F5B731',faint:'rgba(245,183,49,.12)',label:'Moderate Risk'},
  C:{color:'#FF7A45',faint:'rgba(255,122,69,.12)',label:'High Risk'},
  D:{color:'#FF4D6A',faint:'rgba(255,77,106,.12)',label:'Critical Risk'}
};

function getRank(p){return p>=80?'A':p>=60?'B':p>=40?'C':'D'}
...
function toggleTheme(){
  ...
  applyTheme(!isDark);
  logActivity('theme',isDark?'☀️':'🌙','Theme Changed','Switched to '+(isDark?'light':'dark')+' mode');
}

// ══════════════════════════════════
// PREFERENCES
// ══════════════════════════════════
function getPref(k){const p=JSON.parse(localStorage.getItem('cs_admin_prefs')||'{}');return p[k]!==undefined?p[k]:true;}
function savePref(k,v){const p=JSON.parse(localStorage.getItem('cs_admin_prefs')||'{}');p[k]=v;localStorage.setItem('cs_admin_prefs',JSON.stringify(p));}

// ══════════════════════════════════
// ACTIVITY LOG
// ══════════════════════════════════
function getActivity(){return JSON.parse(localStorage.getItem('cs_admin_activity')||'[]');}
function saveActivity(a){localStorage.setItem('cs_admin_activity',JSON.stringify(a.slice(0,100)));}
...
function renderActivityLog(){
  ...
  if(!el)return;
  if(!log.length){el.innerHTML='<div class="activity-empty">No activity recorded yet.</div>';return;}
  const colors={export:'rgba(16,217,130,.12)',flag:'rgba(255,77,106,.12)',refresh:'rgba(245,183,49,.12)',alert:'rgba(255,122,69,.12)',theme:'rgba(91,79,232,.1)',profile:'rgba(91,79,232,.1)'};
...
// NOTIFICATIONS
// ══════════════════════════════════
function getNotifs(){return JSON.parse(localStorage.getItem('cs_admin_notifs')||'[]');}
function saveNotifs(n){localStorage.setItem('cs_admin_notifs',JSON.stringify(n.slice(0,30)));}
...
function closeNotifOutside(e){
  ...
  if(panel&&!panel.contains(e.target)&&e.target!==btn)panel.classList.add('hidden');
}
function renderNotifList(){
  const notifs=getNotifs();
  const el=document.getElementById('notif-list');
  if(!el)return;
  if(!notifs.length){el.innerHTML='<p class="notif-empty">No alerts</p>';return;}
...
function clearNotifs(){
  saveNotifs([]);
  document.getElementById('notif-list').innerHTML='<p class="notif-empty">No alerts</p>';
  renderNotifBadge();
}
...
function onGlobalSearch(query){
  ...
  VENDORS_DATA.forEach(v=>{
    if(v.name.toLowerCase().includes(q)){
      ...
      matches.push({type:'vendor',id:v.id,name:v.name,sub:latest?`Latest: ${latest.pct}% · ${RANK_CFG[latest.rank].label}`:'No assessments'});
    }
  });
  // Search assessments
  getAllAssessments().forEach(a=>{
    if((a.vendorName||'').toLowerCase().includes(q)||String(a.pct).includes(q)){
      if(!matches.find(m=>m.id===a.vendorId)){
        matches.push({type:'assessment',id:a.vendorId,name:a.vendorName,sub:`Score: ${a.pct}% on ${fmtDate(a.date)}`});
      }
    }
  });
  if(!matches.length){panel.innerHTML='<div style="padding:1rem;font-size:.82rem;color:var(--text-3);text-align:center;">No results found</div>';panel.classList.remove('hidden');return;}
  panel.innerHTML=matches.slice(0,8).map(m=>`
...
function checkHighRiskAlerts(){
  ...
  if(critical.length){
    addNotif('🚨','Critical Risk Vendors',`${critical.length} vendor(s) at critical risk: ${critical.slice(0,3).map(v=>v.vendorName).join(', ')}`);
    logActivity('alert','🚨','Critical Risk Alert',`${critical.length} vendor(s) flagged as Critical Risk`);
  }
  if(high.length){
    addNotif('⚠️','High Risk Vendors',`${high.length} vendor(s) at high risk need attention`);
  }
}
...
function getAllAssessments(){
  const all=[];
  VENDORS_DATA.forEach(v=>v.history.forEach(h=>all.push({...h,vendorId:v.id,vendorName:v.name})));
  return all.sort((a,b)=>new Date(b.date)-new Date(a.date));
}
function getLatestPerVendor(){
  return VENDORS_DATA.map(v=>({...v.history[v.history.length-1],vendorId:v.id,vendorName:v.name,totalAssessments:v.history.length,flagged:v.flagged}))
...
function renderStats(){
  const latest=getLatestPerVendor(), all=getAllAssessments(), counts=getRiskCounts(latest);
...
const CHART_FAINT={A:'rgba(34,197,94,.15)',B:'rgba(245,158,11,.15)',C:'rgba(249,115,22,.15)',D:'rgba(239,68,68,.15)'};
const TOOLTIP_CFG={backgroundColor:'#0d1421',borderColor:'rgba(255,255,255,.1)',borderWidth:1,titleColor:'#dde4f0',bodyColor:'#8898b4',padding:10};

function makeBarChart(id,counts,ref){
  const ctx=document.getElementById(id);if(!ctx)return ref;if(ref)ref.destroy();
  return new Chart(ctx,{type:'bar',data:{labels:['A — Low','B — Moderate','C — High','D — Critical'],datasets:[{label:'Vendors',data:[counts.A,counts.B,counts.C,counts.D],backgroundColor:[CHART_FAINT.A,CHART_FAINT.B,CHART_FAINT.C,CHART_FAINT.D],borderColor:[CHART_COLORS.A,CHART_COLORS.B,CHART_COLORS.C,CHART_COLORS.D],borderWidth:2,borderRadius:6,borderSkipped:false}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:TOOLTIP_CFG},scales:{y:{beginAtZero:true,grid:{color:'rgba(59,139,255,.04)'},ticks:{font:{size:10},stepSize:1,color:'#8898b4'}},x:{grid:{display:false},ticks:{font:{size:10},color:'#8898b4'}}}}});
}
function makePieChart(id,counts,ref){
  const ctx=document.getElementById(id);if(!ctx)return ref;if(ref)ref.destroy();
  const total=Object.values(counts).reduce((a,b)=>a+b,0);
  return new Chart(ctx,{type:'doughnut',data:{labels:['A — Low','B — Moderate','C — High','D — Critical'],datasets:[{data:[counts.A,counts.B,counts.C,counts.D],backgroundColor:[CHART_COLORS.A,CHART_COLORS.B,CHART_COLORS.C,CHART_COLORS.D],borderWidth:2,borderColor:'#030508',hoverOffset:6}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:10},padding:12,color:'#8898b4'}},tooltip:{callbacks:{label:ctx=>{const val=ctx.parsed,pct=total?Math.round(val/total*100):0;return ` ${ctx.label}: ${val} (${pct}%)`;}}}}}});
}
function makeLineChart(id,ref){
  const ctx=document.getElementById(id);if(!ctx)return ref;if(ref)ref.destroy();
  const all=getAllAssessments(), byWeek={};
  all.forEach(a=>{const d=new Date(a.date);const week=`${d.getFullYear()}-W${String(Math.ceil((d.getDate()+(new Date(d.getFullYear(),d.getMonth(),1).getDay()||7))/7)).padStart(2,'0')}/${d.toLocaleDateString('en-US',{month:'short',day:'numeric'})}`;if(!byWeek[week])byWeek[week]=[];byWeek[week].push(a.pct);});
  const labels=Object.keys(byWeek).sort().slice(-10);
  const data=labels.map(k=>Math.round(byWeek[k].reduce((a,b)=>a+b,0)/byWeek[k].length));
  const shortLabels=labels.map(l=>l.split('/')[1]);
  return new Chart(ctx,{type:'line',data:{labels:shortLabels,datasets:[{label:'Avg Score %',data,borderColor:'#7B72F0',backgroundColor:'rgba(91,79,232,.1)',fill:true,tension:0.4,pointBackgroundColor:'#7B72F0',pointBorderColor:'#0A0F1E',pointBorderWidth:2,pointRadius:4,pointHoverRadius:7}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:TOOLTIP_CFG},scales:{y:{min:0,max:100,grid:{color:'rgba(59,139,255,.04)'},ticks:{font:{size:10},color:'#8898b4',callback:v=>v+'%'}},x:{grid:{display:false},ticks:{font:{size:10},color:'#8898b4'}}}}});
}
function renderCharts(){const latest=getLatestPerVendor(),counts=getRiskCounts(latest);barChart=makeBarChart('bar-chart',counts,barChart);pieChart=makePieChart('pie-chart',counts,pieChart);lineChart=makeLineChart('line-chart',lineChart);}
function renderReportCharts(){const latest=getLatestPerVendor(),counts=getRiskCounts(latest);barChart2=makeBarChart('bar-chart-2',counts,barChart2);pieChart2=makePieChart('pie-chart-2',counts,pieChart2);}

// ══════════════════════════════════
// ANALYTICS
// ══════════════════════════════════
function renderAnalytics(){
  const latest=getLatestPerVendor(), all=getAllAssessments();
  if(!latest.length)return;
  const avgScore=Math.round(latest.reduce((s,r)=>s+r.pct,0)/latest.length);
  const counts=getRiskCounts(latest), highRisk=(counts.C||0)+(counts.D||0);
  const catTotals={password:0,phishing:0,device:0,network:0}, catCount={...catTotals};
  all.forEach(a=>{if(a.catPct)Object.keys(catTotals).forEach(k=>{catTotals[k]+=a.catPct[k]||0;catCount[k]++;});});
  const catAvg={};Object.keys(catTotals).forEach(k=>catAvg[k]=catCount[k]?Math.round(catTotals[k]/catCount[k]):0);
  const weakest=Object.entries(catAvg).sort((a,b)=>a[1]-b[1])[0];
  const catNames={password:'Password Security',phishing:'Phishing Awareness',device:'Device Safety',network:'Network Safety'};
  const catIcons={password:'🔑',phishing:'🎣',device:'💻',network:'📡'};
  const sorted=[...latest].sort((a,b)=>new Date(a.date)-new Date(b.date));
  const firstHalf=sorted.slice(0,Math.floor(sorted.length/2)), secondHalf=sorted.slice(Math.floor(sorted.length/2));
  const avg1=firstHalf.reduce((s,r)=>s+r.pct,0)/firstHalf.length, avg2=secondHalf.reduce((s,r)=>s+r.pct,0)/secondHalf.length;
  const trendDir=avg2>avg1?'↑ Improving':'↓ Declining', trendColor=avg2>avg1?'#10D982':'#FF4D6A';
...
function renderHeatmap(){
  ...
  if(sortBy==='name')latest.sort((a,b)=>a.vendorName.localeCompare(b.vendorName));
  else if(sortBy==='score')latest.sort((a,b)=>b.pct-a.pct);
  else latest.sort((a,b)=>(b.catPct?.[sortBy]||0)-(a.catPct?.[sortBy]||0));
  ...
  const rows=latest.map(v=>{
    const catCells=cats.map(k=>{
      const pct=v.catPct?.[k]||0, c=heatmapColor(pct);
...
function getFilteredAssessments(filterId){
  ...
  if(filterVal==='CD')return all.filter(r=>r.rank==='C'||r.rank==='D');
...
function renderTable(){
  ...
  if(!rows.length){tbody.innerHTML=`<tr><td colspan="11" style="text-align:center;padding:2rem;color:var(--text-3)">No records found.</td></tr>`;return;}
...
function resetFilter(){document.getElementById('filter-rank').value='';renderTable();}
...
function updateBulkButtons(){
  ...
  const btn=document.getElementById('bulk-action-btn'), flagBtn=document.getElementById('bulk-flag-btn');
...
function bulkExportSelected(){
  ...
  if(!all.length){toast('No rows selected.');return;}
  const headers=['Vendor','Score (%)','Rank','Password %','Phishing %','Device %','Network %','Date'];
  const rows=all.map(r=>[`"${r.vendorName}"`,r.pct,r.rank,r.catPct?.password??'',r.catPct?.phishing??'',r.catPct?.device??'',r.catPct?.network??'',fmtDate(r.date)].join(','));
  const csv=[headers.join(','),...rows].join('\n');
  const blob=new Blob([csv],{type:'text/csv'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download='cybershield_selected_vendors.csv';a.click();
  URL.revokeObjectURL(url);
  toast(`📥 Exported ${ids.size} vendor(s)`);
  logActivity('export','⬇','Bulk Export',`Exported ${ids.size} vendor(s) via bulk selection`);
}

function bulkFlagSelected(){
  const ids=getCheckedVendorIds();
  ids.forEach(id=>{const v=VENDORS_DATA.find(x=>x.id===id);if(v)v.flagged=!v.flagged;});
  localStorage.setItem('cs_admin_vendors',JSON.stringify(VENDORS_DATA));
  renderTable();renderUsersTable();renderHeatmap();
  toast(`🚩 Toggled flag on ${ids.size} vendor(s)`);
  logActivity('flag','🚩','Bulk Flag',`Toggled flag on ${ids.size} vendor(s)`);
}
...
function renderUsersTable(){
  ...
  if(filterVal)vendors=vendors.filter(v=>{const l=v.history[v.history.length-1];return l&&l.rank===filterVal;});
  if(searchVal)vendors=vendors.filter(v=>v.name.toLowerCase().includes(searchVal));
  const tbody=document.getElementById('users-table-body');
  if(!vendors.length){tbody.innerHTML=`<tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--text-3)">No vendors found.</td></tr>`;return;}
...
function toggleFlag(id){
  ...
  v.flagged=!v.flagged;
  localStorage.setItem('cs_admin_vendors',JSON.stringify(VENDORS_DATA));
  renderUsersTable();renderTable();renderHeatmap();
  toast(v.flagged?`🚩 ${v.name} flagged`:`✅ ${v.name} unflagged`);
  logActivity('flag','🚩','Vendor Flag',`${v.name} was ${v.flagged?'flagged':'unflagged'}`);
}
...
function openVendorDetail(vendorId){
  ...
  document.getElementById('modal-title').textContent=v.name+' — Vendor Detail';
  ...
  const historyRows=v.history.map((h,i)=>`
    <tr>
      <td>${i+1}</td>
  ...
  requestAnimationFrame(()=>{
    ...
    if(!ctx)return;
    new Chart(ctx,{type:'line',data:{labels:v.history.map(h=>fmtDate(h.date)),datasets:[{data:v.history.map(h=>h.pct),borderColor:'#7B72F0',backgroundColor:'rgba(91,79,232,.08)',fill:true,tension:0.4,pointRadius:3,pointBackgroundColor:'#7B72F0',pointBorderColor:'#0A0F1E',pointBorderWidth:1}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{...TOOLTIP_CFG,callbacks:{label:ctx=>` ${ctx.parsed.y}%`}}},scales:{y:{min:0,max:100,grid:{color:'rgba(59,139,255,.04)'},ticks:{color:'#8898b4',font:{size:9},callback:v=>v+'%'}},x:{grid:{display:false},ticks:{color:'#8898b4',font:{size:9}}}}}});
  });

  logActivity('view','👁','Vendor Viewed',`Opened detail page for ${v.name}`);
}
...
function exportCSV(){
  ...
  const headers=['Vendor','Score (%)','Rank','Password %','Phishing %','Device %','Network %','Date','Flagged'];
  const rows=all.map(r=>{const v=VENDORS_DATA.find(x=>x.id===r.vendorId);return[`"${r.vendorName}"`,r.pct,r.rank,r.catPct?.password??'',r.catPct?.phishing??'',r.catPct?.device??'',r.catPct?.network??'',fmtDate(r.date),v?.flagged?'Yes':'No'].join(',');});
  const csv=[headers.join(','),...rows].join('\n');
  const blob=new Blob([csv],{type:'text/csv'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download='cybershield_admin_report.csv';a.click();
  URL.revokeObjectURL(url);
  toast('📥 CSV exported!');
...
function exportPDF(){
  ...
  const doc=new jsPDF();
  const latest=getLatestPerVendor(),counts=getRiskCounts(latest);
  const avgScore=latest.length?Math.round(latest.reduce((s,r)=>s+r.pct,0)/latest.length):0;
  const all=getAllAssessments();
  doc.setFillColor(10,15,30);doc.rect(0,0,210,40,'F');doc.setFillColor(91,79,232);doc.rect(0,38,210,2,'F');
  doc.setTextColor(255,255,255);doc.setFontSize(20);doc.setFont('helvetica','bold');doc.text('CyberShield',14,18);
  doc.setFontSize(10);doc.setFont('helvetica','normal');doc.text('Admin Risk Assessment Report',14,28);
  doc.setFontSize(9);doc.setTextColor(148,163,184);doc.text(`Generated: ${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}`,140,28);
  doc.setTextColor(30,30,30);doc.setFontSize(13);doc.setFont('helvetica','bold');doc.text('Executive Summary',14,52);
  doc.setFontSize(10);doc.setFont('helvetica','normal');doc.setTextColor(80,80,80);
  doc.text(`Total Vendors: ${VENDORS_DATA.length}`,14,62);doc.text(`Total Assessments: ${all.length}`,14,70);doc.text(`Average Score: ${avgScore}%`,14,78);
  const flagged=VENDORS_DATA.filter(v=>v.flagged).length;
  doc.text(`Flagged Vendors: ${flagged}`,14,86);
  doc.setFontSize(13);doc.setFont('helvetica','bold');doc.setTextColor(30,30,30);doc.text('Risk Distribution',14,102);
  const riskData=[{rank:'A',label:'Low Risk',count:counts.A,color:[16,217,130]},{rank:'B',label:'Moderate Risk',count:counts.B,color:[245,183,49]},{rank:'C',label:'High Risk',count:counts.C,color:[255,122,69]},{rank:'D',label:'Critical Risk',count:counts.D,color:[255,77,106]}];
  riskData.forEach((r,i)=>{
    const y=112+i*16,total=VENDORS_DATA.length,w=total>0?90*(r.count/total):0;
    doc.setFontSize(10);doc.setFont('helvetica','normal');doc.setTextColor(60,60,60);doc.text(`Rank ${r.rank} — ${r.label}`,14,y);
    doc.setFillColor(20,31,56);doc.rect(80,y-5,90,7,'F');doc.setFillColor(...r.color);doc.rect(80,y-5,w,7,'F');
    doc.setTextColor(30,30,30);doc.text(`${r.count} vendors (${total?Math.round(r.count/total*100):0}%)`,176,y);
  });
  doc.setFontSize(13);doc.setFont('helvetica','bold');doc.setTextColor(30,30,30);doc.text('Vendor Summary (Latest)',14,184);
  doc.setFontSize(9);doc.setFont('helvetica','bold');doc.setTextColor(100,100,100);
  doc.text('Vendor',14,194);doc.text('Score',110,194);doc.text('Rank',140,194);doc.text('Flag',165,194);doc.text('Date',178,194);
  doc.setDrawColor(40,50,70);doc.line(14,196,196,196);
  latest.slice(0,12).forEach((v,i)=>{
    const y=203+i*10;if(y>275)return;
    const vd=VENDORS_DATA.find(x=>x.id===v.vendorId);
    doc.setFont('helvetica','normal');doc.setTextColor(40,40,40);
    doc.text(v.vendorName.substring(0,28),14,y);doc.text(`${v.pct}%`,110,y);doc.text(v.rank,140,y);
    doc.text(vd?.flagged?'🚩':'—',165,y);doc.text(fmtDate(v.date),178,y);
    if(i%2===1){doc.setFillColor(20,28,48);doc.rect(14,y-5,182,9,'F');}
  });
  doc.setFillColor(10,15,30);doc.rect(0,283,210,14,'F');
  doc.setFontSize(8);doc.setTextColor(74,90,122);
  doc.text('Confidential — CyberShield Admin Report',14,291);doc.text('Page 1 of 1',185,291);
  doc.save('CyberShield_Admin_Report.pdf');
  toast('📄 PDF exported!');
  logActivity('export','📄','PDF Export','Admin summary report exported to PDF');
}
...
function renderProfile(){
  ...
  const name=prof.name||'Administrator', email='admin@cybershield.com', org=prof.org||'';
  ...
  const dispEl=document.getElementById('profile-name-display'), dispEmail=document.getElementById('profile-email-display');
  ...
  if(prefDark)prefDark.checked=document.documentElement.getAttribute('data-theme')==='dark';
  const prefAlerts=document.getElementById('pref-alerts');
  if(prefAlerts){prefAlerts.checked=getPref('alerts');prefAlerts.onchange=()=>savePref('alerts',prefAlerts.checked);}
  const prefAR=document.getElementById('pref-autorefresh');
  if(prefAR){prefAR.checked=getPref('autorefresh');prefAR.onchange=()=>savePref('autorefresh',prefAR.checked);}

  // Stats
  const all=getAllAssessments(), latest=getLatestPerVendor();
  const flagged=VENDORS_DATA.filter(v=>v.flagged).length;
  const avg=latest.length?Math.round(latest.reduce((s,r)=>s+r.pct,0)/latest.length):0;
  ...
  ].map(s=>`<div class="profile-stat-item"><div class="label">${s.label}</div><div class="val mono">${s.val}</div></div>`).join('');
}

function saveProfile(){
  const name=document.getElementById('profile-name').value.trim()||'Administrator';
  const org=document.getElementById('profile-org').value.trim();
  localStorage.setItem('cs_admin_profile',JSON.stringify({name,org}));
  const sbName=document.getElementById('sb-admin-name');if(sbName)sbName.textContent=name;
  const dispEl=document.getElementById('profile-name-display');if(dispEl)dispEl.textContent=name;
  const avatarEl=document.getElementById('profile-avatar');if(avatarEl)avatarEl.textContent=name.charAt(0).toUpperCase();
  toast('✅ Profile saved!');
  logActivity('profile','⚙️','Profile Updated',`Admin name updated to "${name}"`);
}
...
Object.assign(SECTION_TITLES, {
  ...
  compliance: ['Compliance Checklist','Per-vendor action items and remediation steps'],
...
switchSection = function(name) {
  ...
  if (name === 'compare')    { populateVendorSelects(); renderComparison(); }
  if (name === 'forecast')   renderForecast();
  if (name === 'compliance') { populateVendorSelects(); renderCompliance(); }
  if (name === 'email')      { populateVendorSelects(); updateEmailPreview(); renderEmailLog(); }
};

function populateVendorSelects() {
  const selects = ['compare-a','compare-b','compliance-vendor','email-vendor'];
  selects.forEach(function(id) {
    ...
    el.innerHTML = VENDORS_DATA.map(function(v, i) {
      const latest = v.history[v.history.length - 1];
      const score = latest ? latest.pct + '% · ' + RANK_CFG[latest.rank].label : 'No data';
...
function renderComparison() {
  ...
  if (!la || !lb) { el.innerHTML = '<p style="color:var(--text-3)">No assessment data available.</p>'; return; }
  ...
  function colColor(id) {
    const colors = ['#7B72F0','#10D982','#F5B731','#FF7A45','#FF4D6A','#9B8FFE'];
  ...
  function catRow(cat) {
    ...
    const colorA = pa >= 80 ? '#10D982' : pa >= 60 ? '#F5B731' : pa >= 40 ? '#FF7A45' : '#FF4D6A';
    const colorB = pb >= 80 ? '#10D982' : pb >= 60 ? '#F5B731' : pb >= 40 ? '#FF7A45' : '#FF4D6A';
    return `
  ...
  const aCfg = RANK_CFG[la.rank], bCfg = RANK_CFG[lb.rank];
  const overallWinner = la.pct > lb.pct ? va.name : lb.pct > la.pct ? vb.name : 'Tie';
  ...
  el.innerHTML = `
    <div class="compare-grid">
      <div class="compare-col">
        ...
        ${cats.map(c => catRow(c)).join('').replace(/compare-cat-row/g,'compare-cat-row a-side')}
      </div>
      ...
      <div class="compare-col">
        ...
        ${cats.map(c => {
          const pa = la.catPct?.[c.key] || 0, pb = lb.catPct?.[c.key] || 0;
          const colorA = pa >= 80 ? '#10D982' : pa >= 60 ? '#F5B731' : pa >= 40 ? '#FF7A45' : '#FF4D6A';
          const colorB = pb >= 80 ? '#10D982' : pb >= 60 ? '#F5B731' : pb >= 40 ? '#FF7A45' : '#FF4D6A';
          const winner = pa > pb ? 'A' : pb > pa ? 'B' : '=';
...
function predictScore(history) {
  if (history.length < 2) return { predicted: history[0]?.pct || 50, trend: 'flat', delta: 0 };
  ...
  const predicted = Math.round(Math.min(100, Math.max(0, intercept + slope * n)));
...
function renderForecast(filter) {
  ...
  const forecasts = vendors.map(v => {
    ...
    const latest = v.history[v.history.length - 1];
    return { ...f, id: v.id, name: v.name, current: latest.pct, currentRank: latest.rank };
  });

  let filtered = forecasts;
  if (filter === 'improving') filtered = forecasts.filter(f => f.trend === 'up');
  else if (filter === 'declining') filtered = forecasts.filter(f => f.trend === 'down');
  else if (filter === 'atrisk') filtered = forecasts.filter(f => f.predicted < 40);

  filtered.sort((a, b) => a.predicted - b.predicted);

  const el = document.getElementById('forecast-grid');
  if (!el) return;
  if (!filtered.length) { el.innerHTML = '<p style="color:var(--text-3);text-align:center;padding:2rem;">No vendors match this filter.</p>'; return; }

  const cards = filtered.map(f => {
    const color = f.predicted >= 80 ? '#10D982' : f.predicted >= 60 ? '#F5B731' : f.predicted >= 40 ? '#FF7A45' : '#FF4D6A';
    const trendLabel = f.trend === 'up' ? `↑ +${Math.abs(f.delta)}%` : f.trend === 'down' ? `↓ ${f.delta}%` : '→ Stable';
    const arrowIcon = f.trend === 'up' ? '📈' : f.trend === 'down' ? '📉' : '➡️';
...
function filterForecast(filter, btn) {
  document.querySelectorAll('.forecast-filter').forEach(b => b.classList.remove('active'));
...
const COMPLIANCE_ITEMS = {
  password: [
    { id:'pw1', title:'Deploy Password Manager', desc:'Mandate use of an approved password manager (e.g. Bitwarden, 1Password) for all staff.', priority:'high' },
    { id:'pw2', title:'Enable MFA on All Accounts', desc:'Enforce multi-factor authentication on email, VPN, and SaaS platforms.', priority:'high' },
    { id:'pw3', title:'Password Policy Review', desc:'Update policy to require minimum 16 characters with complexity requirements.', priority:'medium' },
    { id:'pw4', title:'Quarterly Password Audit', desc:'Schedule automated audits to detect weak or reused credentials.', priority:'low' },
  ],
  phishing: [
    { id:'ph1', title:'Phishing Simulation Training', desc:'Run monthly simulated phishing campaigns and mandatory awareness training.', priority:'high' },
    { id:'ph2', title:'Email Gateway Filtering', desc:'Implement advanced email filtering (DMARC, SPF, DKIM) to block spoofed domains.', priority:'high' },
    { id:'ph3', title:'Incident Response Procedure', desc:'Define clear steps for reporting and responding to suspected phishing attempts.', priority:'medium' },
  ],
  device: [
    { id:'dv1', title:'Full-Disk Encryption', desc:'Enable BitLocker or FileVault on all company devices within 30 days.', priority:'high' },
    { id:'dv2', title:'Patch Management Policy', desc:'Establish automated patching with max 7-day window for critical updates.', priority:'high' },
    { id:'dv3', title:'Screen Lock Policy', desc:'Configure auto-lock after 5 minutes of inactivity on all endpoints.', priority:'medium' },
    { id:'dv4', title:'Mobile Device Management', desc:'Enroll all mobile devices in MDM solution for remote wipe capability.', priority:'low' },
  ],
  network: [
    { id:'nw1', title:'VPN Mandate for Remote Work', desc:'Require company-issued VPN for all access to internal systems from external networks.', priority:'high' },
    { id:'nw2', title:'Network Segmentation', desc:'Implement VLAN segmentation to isolate sensitive systems from general access.', priority:'medium' },
    { id:'nw3', title:'Wi-Fi Security Audit', desc:'Audit and upgrade wireless protocols to WPA3; disable legacy WEP/WPA connections.', priority:'medium' },
    { id:'nw4', title:'USB Policy Enforcement', desc:'Block unknown USB devices via endpoint security policy.', priority:'low' },
  ],
};
...
function toggleChecklistItem(vendorId, itemId) {
  ...
  renderCompliance();
  logActivity('compliance', '✅', 'Compliance Updated', 'Item ' + itemId + ' toggled for vendor ID ' + vendorId);
}
...
function renderCompliance() {
  ...
  const latest = v.history[v.history.length - 1];
  if (!latest) { el.innerHTML = '<p style="color:var(--text-3)">No assessment data for this vendor.</p>'; return; }

  const state = getComplianceState(vendorId);
  const catPct = latest.catPct || {};

  // Determine which categories need attention (score < 70)
  const weakCats = Object.entries(catPct).filter(([k, v]) => v < 70).sort((a, b) => a[1] - b[1]).map(e => e[0]);
...
function updateEmailPreview() {
  ...
  const toEmail  = document.getElementById('email-to')?.value || 'vendor@company.com';
  const subject  = document.getElementById('email-subject')?.value || 'CyberShield Risk Assessment Report';
  ...
  if (!v) return;
  const latest = v.history[v.history.length - 1];
  if (!latest) { el.innerHTML = '<p style="color:var(--text-3)">No assessment data available.</p>'; return; }

  const cfg = RANK_CFG[latest.rank];
  const cats = { password: '🔑 Password', phishing: '🎣 Phishing', device: '💻 Device', network: '📡 Network' };
  const catRows = Object.entries(cats).map(([k, label]) => {
    const pct = latest.catPct?.[k] || 0;
    const color = pct >= 80 ? '#10D982' : pct >= 60 ? '#F5B731' : pct >= 40 ? '#FF7A45' : '#FF4D6A';
...
function sendEmailReport() {
  ...
  const subject  = document.getElementById('email-subject')?.value || 'CyberShield Report';
  const note     = document.getElementById('email-note')?.value || '';
  const v = VENDORS_DATA.find(x => x.id === vendorId);
  if (!v) return;
  if (!toEmail) { toast('⚠ Please enter a recipient email.'); return; }

  const latest = v.history[v.history.length - 1];
  const sentLog = JSON.parse(localStorage.getItem('cs_email_log') || '[]');
  sentLog.unshift({
    vendorName: v.name, to: toEmail, subject, note,
    score: latest?.pct || '—', rank: latest?.rank || '—',
    time: new Date().toISOString()
  });
  localStorage.setItem('cs_email_log', JSON.stringify(sentLog.slice(0, 50)));

  toast('📧 Report sent to ' + toEmail + ' (simulated)');
  addNotif('📧', 'Report Sent', `Assessment report for ${v.name} sent to ${toEmail}`);
  logActivity('export', '📧', 'Email Report Sent', `Report for ${v.name} → ${toEmail}`);
  renderEmailLog();
...
function renderEmailLog() {
  ...
  if (!log.length) { el.innerHTML = '<p style="color:var(--text-3);font-size:.84rem;">No reports sent yet.</p>'; return; }
  el.innerHTML = log.map(l => `
