...
#hm-grid{display:grid;gap:4px;min-width:640px}
.hm-hdr{font-family:var(--mono);font-size:.58rem;letter-spacing:1px;text-transform:uppercase;color:var(--muted);padding:.3rem .4rem;text-align:center}
.hm-label{font-size:.78rem;color:var(--muted2);display:flex;align-items:center;padding:.3rem .6rem;white-space:nowrap;overflow:hidden;max-width:140px}
.hm-cell{height:36px;border-radius:5px;display:grid;place-items:center;font-family:var(--mono);font-size:.68rem;font-weight:600;cursor:pointer;transition:transform .15s,box-shadow .15s}
.hm-cell:hover{transform:scale(1.06);box-shadow:0 4px 12px rgba(0,0,0,.3);z-index:1;position:relative}
.hm-legend{display:flex;align-items:center;gap:.65rem;margin-bottom:1rem;font-size:.72rem;color:var(--muted2)}
.hm-leg-bar{flex:1;max-width:140px;height:8px;border-radius:4px;background:linear-gradient(90deg,var(--red),var(--yellow),var(--green))}
</style>
</head>
<body>
...
<div class="sec-hdr"><h2>Risk Heatmap</h2><p>Per-vendor, per-category score breakdown at a glance.</p></div>
<div class="card" style="padding:1.25rem 1.5rem">
  <div class="hm-legend"><span>Poor</span><div class="hm-leg-bar"></div><span>Excellent</span></div>
  <div style="overflow-x:auto"><div id="hm-grid"></div></div>
</div>
...
function pageInit(){
  ...
  const vendors=MOCK.vendors.slice(0,10);
  const cols=cats.length;
  const grid=document.getElementById('hm-grid');
  grid.style.gridTemplateColumns=`160px repeat(${cols},1fr)`;
  let html='<div class="hm-hdr"></div>';
  cats.forEach(c=>html+=`<div class="hm-hdr">${c.split(' ').slice(0,2).join(' ')}</div>`);
  vendors.forEach(v=>{
    html+=`<div class="hm-label">${v.name.split(' ')[0]}</div>`;
    cats.forEach(()=>{
      const s=Math.round(Math.random()*75+20);
      const color=s>=75?'#10D982':s>=55?'#F5B731':s>=35?'#FF8C42':'#FF4D6A';
      const op=(0.2+(s/100)*0.7).toFixed(2);
      html+=`<div class="hm-cell" style="background:${color};opacity:${op}" title="${s}%" onclick="showToast('Score: ${s}%','blue')">${s}%</div>`;
    });
  });
  grid.innerHTML=html;
}
