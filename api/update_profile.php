...
try {
    ...
    if ($stmt->execute()) {
        ...
        $changes = [];
        if ($user['full_name'] !== $full_name) $changes[] = "name to '$full_name'";
        if ($user['email'] !== $email) $changes[] = "email to '$email'";
        if ($user['store_name'] !== $store_name) $changes[] = "store name to '$store_name'";
